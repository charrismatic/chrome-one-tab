...
function YK(b8, E8, X8) {
  ...
  A8.type = b8;
  A8.Yn = pK.gW(E8);
  window['chrome']['runtime']['sendMessage'](undefined, A8, undefined, function (A6) {
    var X6 = {};
    if (A6 && A6.Yn) {
      X6 = pK.parse(A6.Yn)
    }
...
function I1(nb, J8, G8) {
  ...
  W8.src = nb ? 'images/twister-open.png' : ('images/twister-closed' + (zj() ? '' : '-rtl') + '.png');
      ...
      for (fb = 0; fb < Rb; fb += 1) {
        ...
        if (Tb >= ' ') {
          ...
          default:
            ;
            Tb = Tb.charCodeAt();
            Zb += '\\u00' + Math.floor(Tb / 16).toString(16) + (Tb % 16).toString(16)
          }
        }
      }
      return Zb + '\"';
...
mj.prototype.oW = function (vb, wb, gb, Nb, zb, ob, ub, Sb, cb) {
  ...
  vb.onmousedown = function (e_) {
    var l_ = false;
    OX(e_, vb, null, function (yN, jN, mN, CN) {
      if (cb) {
        l_ = cb()
      }
...
function gX(Ef) {
  ...
  if (Af == 0) {
    ...
    Xf.appendChild(cK());
    if (vf != Ef.length) Xf.appendChild(document.createTextNode(Ef.substring(vf)))
  }
  return Xf
};
...
function fK(IS) {
  ...
  if (AK && (IS instanceof TouchEvent)) {
    ...
    if (nK || mK) {
      CS = window.event.clientX + (mK ? 0 : document.documentElement.scrollLeft) + document.body.scrollLeft;
      mS = window.event.clientY + (mK ? 0 : document.documentElement.scrollTop) + document.body.scrollTop
    }
    else {
...
function nj($LS, QLS, YLS) {
  return function () {
    for (var N_ in QLS)
      if (window.event.toElement == QLS[N_]) return;
...
function f1(rLS, ZLS) {
  try {
    if (!ZLS) return false;
    while (ZLS.parentNode)
      if ((ZLS = ZLS.parentNode) == rLS) return true;
    return false
  }
  catch (HLS) {
    return false
...
function i1(oLS) {
  if (nK || mK) {
    var uLS = window.event.clientX + (mK ? 0 : document.documentElement.scrollLeft) + document.body.scrollLeft;
    var zLS = window.event.clientY + (mK ? 0 : document.documentElement.scrollTop) + document.body.scrollTop
  }
  else {
...
function u1(zXS) {
  ...
  b1(UXS, '36px');
  UXS.appendChild(I1(false, x1('importUrls'), function () {
  ...
  UXS.appendChild(I1(true, x1('exportUrls'), function () {
